...
// a flat background, an accent fill growing from the left by the value fraction, and the value
// label centered on top. Several of these compose inside a custom control (e.g. the morph pad's
...
public:
    ...
    void draw (IGraphics& g, const IRECT& rect, float frac,
        ...
        if (frac > 0.f)
            g.FillRect (_accent, rect.GetFromLeft (rect.W() * frac), blend);
