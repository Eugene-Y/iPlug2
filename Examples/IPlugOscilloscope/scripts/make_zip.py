...
def  main():
  ...
  installer = "\installer\IPlugOscilloscope Installer.exe"
   
  if demo:
    installer = "\installer\IPlugOscilloscope Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugOscilloscope manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugOscilloscope-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugOscilloscope-v" + FULLVERSIONSTR + "-win-demo.zip"
